...
function syncGeoMarkers(L, map, ref, points, activeIndex, onMarkerClick) {
  ...
  ref.list = clusterGeoPoints(points).map((c) => {
    ...
    const marker = L.marker([c.lat, c.lon], {
      ...
      // независимо от широты (обычный порядок отрисовки Leaflet — по Y-координате пикселя).
      // 1000 недостаточно: Leaflet считает итоговый z-index как "Y-пиксель маркера + offset",
      // и на высоком окне соседняя капля южнее (её Y-пиксель больше) всё равно перебивала
      // активную, если та стояла ближе к верху карты. Разница в Y между каплями на экране
      // больше пары тысяч пикселей не бывает, поэтому берём offset с большим запасом.
      zIndexOffset: active ? 100000 : 0,
    });
    if (onMarkerClick) {
      marker.on("click", (e) => {
